...
import org.xslife.LocalChurch.db.DBHelper;
import android.content.Context;
import android.database.Cursor;
import android.database.SQLException;
import android.database.sqlite.SQLiteDatabase;

// DownloadInfo FileState 
public class DownloadInfoMgr {
	...
	public DownloadInfoMgr(Context context) {
		...
		synchronized (file_Lock) {
			...
			try {
				...
				Object[] bindArgs = { fileState.getFileName(),
						fileState.getUrl(), fileState.getFileSize(), };
				database.execSQL(sql, bindArgs);
				database.setTransactionSuccessful();
			} catch (SQLException e) {
				e.printStackTrace();
	...
	public void delete(String url) {
		...
		database.execSQL("DELETE FROM thread_tab WHERE url = ? ",
				new Object[] { url });
		database.close();
	}
	...
	public void delete(String url, int threadid) {
		...
		db.execSQL("DELETE FROM thread_tab WHERE url = ? AND thread_id = ?",
	...
	public void deleteFileState(String fileName) {
		SQLiteDatabase database = dbHelper.getReadableDatabase();
		database.delete("localfile_tab", "file_name=?",
				new String[] { fileName });
		database.close();
	}
